...
class DefaultClient:
    ...
    def execute(self, request=None):
        try:
            ...
            response = Response(host=request.get_host(), url=request.get_url(), method=request.get_method(),headers=headers, protocol=request.get_protocol(), content_type=request.get_content_type(),content=request.get_body(), time_out=request.get_time_out())
    ...
    def build_headers(self, request=None):
        ...
        if request.get_content_type():
            ...
            headers[constant.HTTP_HEADER_CONTENT_TYPE] = header_params[constant.HTTP_HEADER_CONTENT_TYPE]
        else:
        ...
        if constant.POST == request.get_method() and constant.CONTENT_TYPE_STREAM == request.get_content_type():
            ...
            str_to_sign = signature_composer.build_sign_str(uri=request.get_url(), method=request.get_method(),headers=headers)
        else:
            str_to_sign = signature_composer.build_sign_str(uri=request.get_url(), method=request.get_method(),headers=headers, body=body)
